...
const client = new Rzp({
  username: process.env['RZP_USERNAME'], // This is the default and can be omitted
...
await client.paymentLinks.create({ amount: 1000, currency: 'INR', description: 'Payment for policy no' }, {
  maxRetries: 5,
});
```
...
const response = await client.paymentLinks
  .create({ amount: 1000, currency: 'INR', description: 'Payment for policy no' })
  .asResponse();
console.log(response.headers.get('X-My-Header'));
console.log(response.statusText); // access the underlying Response object

const { data: paymentLink, response: raw } = await client.paymentLinks
  .create({ amount: 1000, currency: 'INR', description: 'Payment for policy no' })
  .withResponse();
...
1. Via the `RZP_LOG` environment variable
