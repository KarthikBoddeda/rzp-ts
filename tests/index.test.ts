...
import { APIPromise } from 'rzp/core/api-promise';

import util from 'node:util';
import Rzp from 'rzp';
import { APIUserAbortError } from 'rzp';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    const forceAPIResponseForClient = async (client: Rzp) => {
    ...
    test('default logLevel is warn', async () => {
      const client = new Rzp({ username: 'My Username', password: 'My Password' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['RZP_LOG'] = 'debug';
      const client = new Rzp({ logger: logger, username: 'My Username', password: 'My Password' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['RZP_LOG'] = 'not a log level';
      const client = new Rzp({ logger: logger, username: 'My Username', password: 'My Password' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'RZP_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['RZP_BASE_URL'] = undefined;
    });
    ...
    test('env variable', () => {
      process.env['RZP_BASE_URL'] = 'https://example.com/from_env';
      const client = new Rzp({ username: 'My Username', password: 'My Password' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['RZP_BASE_URL'] = ''; // empty
      const client = new Rzp({ username: 'My Username', password: 'My Password' });
      expect(client.baseURL).toEqual('https://api.razorpay.com/v1');
    });

    test('blank env variable', () => {
      process.env['RZP_BASE_URL'] = '  '; // blank
      const client = new Rzp({ username: 'My Username', password: 'My Password' });
      expect(client.baseURL).toEqual('https://api.razorpay.com/v1');
    });
    ...
    test('in request options overridden by env variable', () => {
      process.env['RZP_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Rzp({ username: 'My Username', password: 'My Password' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Rzp({ maxRetries: 4, username: 'My Username', password: 'My Password' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Rzp({ username: 'My Username', password: 'My Password' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['RZP_USERNAME'] = 'My Username';
    process.env['RZP_PASSWORD'] = 'My Password';
    const client = new Rzp();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['RZP_USERNAME'] = 'another My Username';
    process.env['RZP_PASSWORD'] = 'another My Password';
    const client = new Rzp({ username: 'My Username', password: 'My Password' });
    expect(client.username).toBe('My Username');
    expect(client.password).toBe('My Password');
  });
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Rzp({ username: 'My Username', password: 'My Password', fetch: testFetch });
